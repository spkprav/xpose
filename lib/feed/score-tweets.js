...
let CONFIG = {
  ...
  // qwen2.5vl:7b for text too — llama3:8b returned identical canned scores across
  // unrelated tweets. qwen2.5vl handles both text and vision in one model.
  textModel:   process.env.OLLAMA_MODEL_SCORE  || 'qwen2.5vl:7b',
  visionUrl:   process.env.OLLAMA_VISION_URL   || null,
...
async function captionImages(imageUrls) {
  ...
  for (const url of imageUrls) {
    try {
      ...
      const visionPrompt = `Describe this image in 1-2 sentences. Then on a new line, classify it as ONE of:
  TYPE: code           (code editor, terminal, IDE screenshot)
  TYPE: product-ui     (dashboard, app UI, settings panel from real software)
  TYPE: chart-data     (graph, metric, benchmark, analytics screenshot)
  TYPE: tweet-screenshot   (a screenshot of someone else's tweet)
  TYPE: meme           (joke image, parody, fake card/UI mockup, satire)
  TYPE: lifestyle      (food, travel, selfie, animals, family)
  TYPE: branding       (marketing creative, logo, ad, generic stock-style)
  TYPE: text-image     (image is just stylized text or quote)
  TYPE: other          (none of the above)
Be honest about the type. A fake credit card or fake stock screenshot is meme, not chart-data.`;
...
function buildPrompt(icp, tweet, content, caption) {
  ...
  const textIsEmpty = !(content.text || '').trim();
...
"${(content.text || '').slice(0, 600)}"${textIsEmpty ? '\n(NOTE: tweet text is empty — score must rely entirely on image content below)' : ''}${quotedBlock}${imageBlock}${replyToBlock}

CONTEXT: ${ageMin == null ? 'unknown age' : ageMin + ' min old'}, ${stats}

══════════════════════════════════════════════════════════════════
HARD DISQUALIFIERS (apply BEFORE picking scores):

If IMAGE CONTENT is "TYPE: meme" or "TYPE: lifestyle" or "TYPE: branding"
AND the tweet text does not contain a specific dev/AI/builder/product topic
→ icp_fit MUST be ≤ 18, signal_quality MUST be ≤ 22, reply_opportunity ≤ 25.
A fake credit card, parody UI, joke screenshot is a MEME — not product content.

If tweet text is empty / just a URL AND image is meme/lifestyle/branding
→ this is a low-value joke post. icp_fit ≤ 12.

If you cannot point to a SPECIFIC indie-builder / AI-shipping / dev-tool
keyword in the text or image, icp_fit MUST be < 30.

EVIDENCE RULE:
The "reason" field MUST quote a SPECIFIC phrase from THIS tweet's text or
image content that drove your icp_fit score. Do NOT copy example phrases.
Format:
  high score → reason: "high icp_fit: '<exact phrase from this tweet>'"
  low score  → reason: "low icp_fit: <accurately describe THIS tweet's actual content>"

DO NOT echo phrases from these instructions (e.g. do not write "credit
card meme" if the tweet isn't about a credit card). Describe what is
ACTUALLY in front of you.

Generic phrases like "aligns with ICP", "invites a reply", "high signal"
WITHOUT a tweet-specific quote → you have FAILED. Lower all scores.
══════════════════════════════════════════════════════════════════

Score 6 criteria. Each: INTEGER 0-100. Use the FULL range — pick numbers
like 27, 53, 71, 84. Do NOT default to multiples of 10.

ANCHORS:

1. icp_fit — does this TWEET'S CONTENT match the user's ICP?
    5  = meme / joke / fake mockup with no builder substance
...
2. reply_opportunity — does this TWEET invite a reply that lands?
    8  = pure meme, only valid replies are 'lol' or emoji
    11 = closed statement, no hook
...
4. signal_quality — substance vs shitpost / motivation / lifecoach noise
    6  = meme, joke, parody mockup, no information
...
5. voice_match — would replying align with user's voice (concrete, smart, grounded)?
    12 = topic forces vague/inspirational reply OR meme reply
...
{"icp_fit":<int>,"reply_opportunity":<int>,"freshness_window":<int>,"signal_quality":<int>,"voice_match":<int>,"virality_trajectory":<int>,"reason":"<MUST quote specific phrase from tweet/image OR explain why no relevant content found, ≤ 25 words>"}`;
}
